package com.mindbees.expenditure.ui;
...
public class CircularContactView extends ViewSwitcher
  ...
  private boolean isTextSmall = false;
  ...
  public CircularContactView(final Context context,final AttributeSet attrs)
    ...
    if(isInEditMode())
      setTextAndBackgroundColor("",0xFFff0000, false);
    }
  ...
  private void drawContent(final int viewWidth,final int viewHeight)
    ...
    if(mImageResId!=0)
      ...
      if (isTextSmall) {
    	  mTextView.setTextSize(viewHeight/9);
	}else {
		mTextView.setTextSize(viewHeight/6);
	}
  ...
  public void setTextAndBackgroundColor(final CharSequence text,final int backgroundColor, boolean isTextSmall)
    {
